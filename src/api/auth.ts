...
export const verifyGoogleToken = async (token: any) => {
    const res = await API_INSTANCE.get(`https://www.googleapis.com/oauth2/v1/userinfo?access_token=${token}`)
    return res
}

export const signout = async (data: any) => {
    const res = await API_INSTANCE.post("/user/signout", data, {
        headers: {
...
export const getCurrentUser = async (token: string) => {
    const res = await API_INSTANCE.post("/auth/me", {}, {
        headers: {
